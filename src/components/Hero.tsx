...
function Hero() {
  ...
  const [delay, setDelay] = useState(0); // Crée une référence pour savoir si c'est le premier rendu
  const [languageChanged, setLanguageChanged] = useState(false); // Crée une référence pour savoir si la langue a changé

  const { t, i18n } = useTranslation();

  // USeEffect pour définir le texte à taper
  useEffect(() => {
    setTextToType(t("hero.job"));

    if (languageChanged) {
      setDelay(0); // Si la langue a été changée, définir le délai à 0ms
    } else {
      setDelay(2000); // Sinon, définir le délai à 2000ms
    }
  }, [i18n.language, t, languageChanged]);

  // Écoute les changements de langue
  useEffect(() => {
    const handleLanguageChange = () => {
      setLanguageChanged(true); // Marquer que la langue a été changée
    };
    ...
    >
      ...
      <h2 className="mb-2 font-rubik text-xl font-semibold text-orange sm:text-2xl md:text-3xl">
        <Typewriter
          ...
          options={{
            autoStart: true, // Désactive le démarrage automatique
            loop: false,
            cursor: "", // Enlève le curseur
          }}
        />
